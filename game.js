...
// --- PERFILES DE CALIDAD (para rendimiento) ---
...
if (isTouchDevice) {
  qualityProfile = QUALITY_LOW; // en móviles, arrancamos en calidad baja optimizada
}
...
// LÍMITES DE EFECTOS SEGÚN PERFIL
const MAX_PARTICLES = qualityProfile === QUALITY_LOW ? 60 : 140;
const MAX_STREAKS = qualityProfile === QUALITY_LOW ? 30 : 80;
const MAX_TRAIL = qualityProfile === QUALITY_LOW ? 20 : 45;
const MAX_COMBO_TEXTS = qualityProfile === QUALITY_LOW ? 12 : 25;

// Estrellas en capas (parallax simple)
function getStarLayersConfig() {
  if (qualityProfile === QUALITY_LOW) {
    return [
      { count: 24, speed: 0.16, radius: 1.3, alpha: 0.26 }
    ];
  }
  if (qualityProfile === QUALITY_MEDIUM) {
    return [
      { count: 30, speed: 0.08, radius: 1.0, alpha: 0.20 },
      { count: 20, speed: 0.16, radius: 1.6, alpha: 0.30 }
    ];
  }
  return [
...
const STAR_LAYERS_CONFIG = getStarLayersConfig();
...
// Toggle de hardcore compartido (teclado + botón táctil)
...
if (isTouchDevice) {
  ...
  // Botón táctil: Reiniciar / Empezar (equivalente a ESPACIO)
  ...
  // Botón táctil: Hardcore (equivalente a tecla H)
...
function spawnBlockParticles(obstacle, colorOverride) {
  ...
  const count = qualityProfile === QUALITY_LOW ? 6 : 12;
  ...
  // Limitar cantidad máxima de partículas
  const extra = particles.length - MAX_PARTICLES;
  if (extra > 0) {
...
function update(deltaTime) {
  ...
  if (playerTrail.length > MAX_TRAIL) {
    playerTrail.splice(0, playerTrail.length - MAX_TRAIL);
  }
  ...
  obstacles.forEach(o => {
    ...
    if (!isGameOver && horizontalOverlap && isNearVertically && !o._nearMissTriggered) {
      ...
      if (streaks.length > MAX_STREAKS) {
        streaks.splice(0, streaks.length - MAX_STREAKS);
      }
      ...
      if (comboTexts.length > MAX_COMBO_TEXTS) {
        comboTexts.splice(0, comboTexts.length - MAX_COMBO_TEXTS);
      }
...
function draw() {
  ...
  // Scanlines (solo en calidad media/alta)
  if (qualityProfile !== QUALITY_LOW) {
    ctx.save();
  ...
  // Motion field diagonal (desactivado en LOW)
  if (qualityProfile !== QUALITY_LOW) {
  ...
  const enableShadows = qualityProfile !== QUALITY_LOW;
  ...
  // Obstáculos con gradiente y bloom dorado (bloom reducido en LOW)
  obstacles.forEach(o => {
    ...
    // Bloom extra para dorados (solo en media/alta)
  ...
  // Nivel + combo (derecha) con highlight en level up
  ctx.save();
  ctx.textAlign = "right";
